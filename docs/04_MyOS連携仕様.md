...
- セッション: 筋トレ・有酸素・イベント参加のいずれかの実績がある記録1件を1セッションと数える
- minutes: 有酸素の合計時間 + イベントの実働時間 + 秒数ベースのセット(プランク等)を分に換算
  (重量×回数のセットは時間を持たないため含めない)
- イベントの実働時間: `bouts`(何分×何本)があればその合計、なければ `durationSec`
- title: 筋トレ種目名・有酸素種別・イベント名を「・」で連結(重複除去)。いずれもなければ「トレーニング」
- `events` を持たない旧ペイロード(KV に残っている送信済みデータ)は空配列として扱う
