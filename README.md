...
- イベント参加(サッカー等): 開始時刻・実施時間・出場時間の内訳(何分×何本)・きつさ (RPE 1〜10)・距離・メモを記録し、実働時間と推定負荷 (AU = 実働分 × RPE) を算出
- 記録の一覧・編集・削除、直近7日間サマリー(日数・総ボリューム・有酸素時間・イベント時間)
- 種目マスタ: トレーニング種目を登録して選択式で入力(既存記録から自動生成・連携タブで管理)
- MyOS 連携: JSON エクスポート / Webhook 送信(連携タブで設定)/ 参照 API(`api/`、MyOS がポーリング)
- 目標設定: 今日・今週・今月の目標(総ボリューム・有酸素時間・トレーニング日数)と達成度の閲覧、日・週・月ごとの達成度履歴(直近7日 / 8週 / 6ヶ月)の管理(目標タブ)
